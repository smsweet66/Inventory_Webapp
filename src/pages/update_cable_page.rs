...
use crate::store::{Store, set_page_loading, set_show_alert, update_cable};
...
fn get_input_callback(field: Field, cloned_form: UseStateHandle<Cable>) -> Callback<String> {
...
fn get_button_callback(field: Field, cloned_form: UseStateHandle<Cable>) -> Callback<MouseEvent> {
...
pub struct UpdateCablePageProps {
	pub cable_id: i32,
}
...
#[function_component]
pub fn UpdateCablePage(props: &UpdateCablePageProps) -> Html {
	let (store, dispatch) = use_store::<Store>();
	let form = use_state(|| store.cables.iter().find(|cable| cable.id == props.cable_id).unwrap().clone());
			...
			spawn_local(async move {
				match form.validate() {
					Ok(_) => {
						...
						let response = cable_requests::update_cable(&data).await;
						match response {
							Ok(cable) => {
								set_page_loading(false, dispatch.clone());
								update_cable(cable, dispatch);
								navigator.go(-1);
							},
							Err(err) => {
								set_page_loading(false, dispatch.clone());
